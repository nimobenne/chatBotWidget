...
class SupabaseStore implements Store {
  ...
  async upsertConversation(input: {
    ...
    const currentTranscript = ((existing.data as ConversationRow | null)?.transcript || []).slice(-100);
    const transcript = [...currentTranscript, ...input.newMessages];
    ...
    const fullUpsert = await supabase.from('conversations').upsert(fullPayload, { onConflict: 'business_id,session_id' });
    if (!fullUpsert.error) return;

    const missingColumn = fullUpsert.error.message.toLowerCase().includes("could not find the 'last_assistant' column") ||
      fullUpsert.error.message.toLowerCase().includes("could not find the 'last_user' column");

    if (!missingColumn) {
      throw new Error(`Failed to save conversation: ${fullUpsert.error.message}`);
    }

    const fallbackUpsert = await supabase.from('conversations').upsert(
      {
        business_id: input.business_id,
        session_id: input.session_id,
        transcript,
    ...
    if (fallbackUpsert.error) {
      throw new Error(`Failed to save conversation: ${fallbackUpsert.error.message}`);
    }
  }
